...
| `--color-red-500` | *(not used in blocks; alias --btn-color-red is used)* |
...
| `--color-blue-200` | *(not used in blocks)* |
| `--color-blue-800` | *(not used directly; --text-color-hero-heritage-cc uses it)* |
...
| `--color-gray-400` | *(not used in blocks)* |
| `--color-gray-500` | header/header.css:1731 |
| `--color-gray-700` | *(not used directly; --text-color uses it)* |
...
| `--color-gradient-pewter-stops` | *(already in --gradient-pewter)* | cards:1292; hero-heritage-cc:448 — use `var(--gradient-pewter)` or keep gradient |
